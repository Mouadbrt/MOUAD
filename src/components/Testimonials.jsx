      ...
      <div className="mt-10 flex gap-5 overflow-x-auto scroll-row pb-4 -mx-1 px-1" style={{ scrollSnapType: "x mandatory" }}>
        {testimonials.map((item) => (
          ...
          >
            ...
            <p className="mt-3 text-sm text-ink/70 italic leading-relaxed">{item.quote}</p>
            <p className="mt-5 font-display text-xs font-bold uppercase tracking-widest text-ink/70">{item.name}</p>
          </div>
        ))}
      </div>
    </section>
